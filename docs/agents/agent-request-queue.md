...
`reject` 用于不接受排队的调用。线程忙碌时，系统记录并返回拒绝状态，不创建 Agent 运行。
...
当前阶段在 Agent 运行成功完成后自动派发下一条请求。运行失败、被取消或进入需要人工处理的中断状态时，不自动派发后续请求。

该规则意味着后续请求仍保留在队列中，但需要先处理当前线程的异常或中断状态。对于前后请求存在依赖关系的场景，这可以避免在前置结果不可用时继续执行。

如果后续调整为在所有终态后继续派发，需要同时明确失败后的上下文、消息展示和依赖请求处理方式。

## 持久化与恢复

请求、输入消息和派发关系保存在数据库中。浏览器刷新后，前端可以重新读取当前线程的排队请求和位置。

Agent 运行由后台任务系统执行。为处理“数据库已经记录派发，但任务尚未成功投递”这一故障窗口，服务启动时会扫描需要恢复的运行并重新投递。恢复过程复用已有请求和运行记录，不创建重复运行。
