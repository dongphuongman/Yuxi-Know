...
- 新增智能体请求队列（Phase 1）：同一线程运行中提交的新请求默认持久化排队，并按 FIFO 顺序自动执行；支持实时查看排队位置、单条取消和刷新恢复，也可通过 `reject` 策略保持忙碌时拒绝。聊天、Agent Call 和评估统一接入该队列，前端会分开展示排队请求与当前回复，并修复连续请求交接时的流状态和消息顺序问题；同步 Agent Call 遇到忙线程时会直接返回拒绝结果，不再因缺少 `run_id` 进入等待逻辑并返回 500。新增《Agent 请求队列与调度设计》文档，说明调度目标、策略、状态、异常处理和当前范围。补充队列、取消、消息排序、流交接及接口测试。
